...
<%
	...
	// 입력한 id와 pass가 동일하고 id가 공백이 아니면 쿠키 생성 - response.addCookie()
	if (id.equals(pass) && !id.equals("")) {
		response.addCookie( // 로그인 상태 판단을 위해 사용
			CookieUtil.createCookie("LOGIN", "SUCCESS", "/", -1) // -1 : 무한데  로그인 성공을 쿠키에 저장
		);
		response.addCookie( // 로그인한 아이디 저장
			CookieUtil.createCookie("ID", id, "/", -1) // 유저가 입력한 아이디를 쿠키에 저장
		);
%>
로그인에 성공하였습니다.<br>
<%
